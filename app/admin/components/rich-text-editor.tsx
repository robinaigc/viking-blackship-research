...
export function RichTextEditor({ initialContent, onChange }: Props) {
  ...
  const editor = useEditor({
    ...
    onUpdate: ({ editor: currentEditor }) => onChange(currentEditor.getJSON() as ArticleDocument),
  });
  ...
  if (!editor) return <div className="min-h-[460px] rounded-lg border border-zinc-800 bg-black/30" />;
  ...
  return (
    <div>
      <div className="flex flex-wrap gap-2 rounded-t-lg border border-zinc-700 bg-zinc-900 p-3">
        ...
        <button type="button" className={buttonClass} onClick={() => editor.chain().focus().toggleHeading({ level: 3 }).run()}>H3</button>
        <button type="button" className={buttonClass} onClick={() => editor.chain().focus().toggleBulletList().run()}>Bullets</button>
        <button type="button" className={buttonClass} onClick={() => editor.chain().focus().toggleOrderedList().run()}>Numbers</button>
        <button type="button" className={buttonClass} onClick={() => editor.chain().focus().toggleBlockquote().run()}>Quote</button>
        <button
          type="button"
        ...
        >Link</button>
        <button type="button" className={buttonClass} disabled={uploading} onClick={() => fileInput.current?.click()}>
          {uploading ? "Uploading..." : "Image"}
        </button>
        <button type="button" className={buttonClass} onClick={() => editor.chain().focus().insertTable({ rows: 3, cols: 3, withHeaderRow: true }).run()}>Table</button>
        <button type="button" className={buttonClass} onClick={() => editor.chain().focus().addRowAfter().run()}>+ Row</button>
        <button type="button" className={buttonClass} onClick={() => editor.chain().focus().addColumnAfter().run()}>+ Column</button>
        <button type="button" className={buttonClass} onClick={() => editor.chain().focus().deleteTable().run()}>Delete table</button>
